...
async function setup() {
  ...
  document.getElementById("episodeDropdown").style.display = "inline-block";
  document.getElementById("episodeCount").style.display = 'inline';
}else{
...
async function renderCards(dataArray, type) {
  const root = document.getElementById("root");
  const countDisplay = document.getElementById("episodeCount");
  ...
  dataArray.forEach((item) => {
    ...
    if (type === "episode") {
      ...
      img.src = item.image ? item.image.medium : "placeholder.jpg";
      ...
      anchor.addEventListener("click", async (e) => {
        e.preventDefault();
        await loadEpisodesForShow(item.id); // Fetch episodes for this show
      });
    } else {
  ...
  // Scroll to top of rendered cards
  root.scrollIntoView({ behavior: "smooth", block: "start" });

  // Update episode/show count
  if (type === "episode") {
    countDisplay.textContent = `Showing ${dataArray.length} episodes`;
  } else if (type === "show") {
